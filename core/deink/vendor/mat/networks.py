"""MAT (Mask-Aware Transformer) generator network.

Vendored (verbatim slab) from IOPaint's ``iopaint/model/mat.py``
(https://github.com/Sanster/IOPaint, Apache-2.0) — the MAT architecture by Li et al.,
"MAT: Mask-Aware Transformer for Large Hole Image Inpainting" (CVPR 2022,
https://github.com/fenglinglwb/MAT). Only the inference-time network (the ``Generator``
and its submodules) is kept; the training discriminator variants and the iopaint
``InpaintModel`` runner wrapper are dropped — the runner lives in ``__init__.py``.
All StyleGAN ops come from the sibling pure-PyTorch ``ops`` module (no CUDA build).
"""
