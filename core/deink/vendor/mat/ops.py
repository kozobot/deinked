"""StyleGAN2 / MAT primitive ops — pure-PyTorch reference implementations.

Vendored (verbatim slab) from IOPaint's ``iopaint/model/utils.py``
(https://github.com/Sanster/IOPaint, Apache-2.0), which itself ports the NVIDIA
StyleGAN2-ADA ops as pure-PyTorch fallbacks: every op ignores its ``impl="cuda"``
argument and runs the ``_ref`` path, so **no custom CUDA extension is built**.
Only the subset MAT needs is copied here — the diffusion/scheduler helpers that
surround it in the original (and pull in ``diffusers``) are deliberately omitted,
